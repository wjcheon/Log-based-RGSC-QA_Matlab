...
filename = "\\10.10.37.115\Research\0.wjcheon\00_Research\07_GatingQA @ynkang\EPID video\trial_1\signal_240624_1.txt"
% filename = "\\10.10.37.115\Research\0.wjcheon\00_Research\07_GatingQA @ynkang\EPID video\trial_2\signal_240624_2.txt"
...
try
    while ~feof(fileID)
        ...
        if strcmp(lineTemp, beamFlagName)
            ...
            dataStruct.LR = [beam_time; beam_Amp];
            % You can add more sections here following the same pattern
        end
    end
catch
...
%
figure, plot(beam_status);
plot(AP_Amp), title('AP'), hold on
plot(beam_status.*0.07), title('Beam on/off')

%% Generates averaged periodic signal
% AP_Amp = AP_Amp(1:3740)
% AP_time = AP_time(1:2020);
% beam_status = beam_status(1:2020);
...
for iter1=1:numPeriods
    ...
    aa = [aa, originalIndicesPhase_temp(starIndex75) - originalIndicesPhase_temp(starIndex25)]
    ...
    beam_on_duraion_per_period = [beam_on_duraion_per_period, (endBeamStatus-startBeamStatus).*x_step_second]
...
sgtitle(title_temp)
saveas(hhh, "result3.png")
% sgtitle("hi")
% title("??????? ??: <0.1 s")

%%
sprintf("st_mean: %1.4f, st_std: %1.4f",mean(diffSetSt, 1), std(diffSetSt, 1) )
sprintf("end_mean: %1.4f, end_std: %1.4f",mean(diffSetEnd, 1), std(diffSetEnd, 1) )
